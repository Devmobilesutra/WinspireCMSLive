...
/**
 * Created by zogato on 29/2/17.
 */
public abstract class CallManagerAsyncTask extends AsyncTask<Object, String, JSONObject> {
    ...
    public CallManagerAsyncTask(String action , String reqType , Context context){

        try {
            this.reqType = reqType;
            this.action  = action;
            url = new URL(Constants.Base_url + action);
   ...
   private void doGet() throws ProtocolException {
    ...
    protected JSONObject doWork(JSONObject inputJSONDATA) throws ConnectException, EOFException, SocketTimeoutException {
        ...
        try {
            ...
            Utils.Log("MSg response === " + urlConnection.getResponseMessage().toString());
            ...
            {
                responseJSON = convertResponseToJSON(urlConnection.getErrorStream());
            }
...
//        Utils.Log("Response==="+responseJSON.toString());
    ...
    protected JSONObject doWorkPOST(JSONObject inputJSONDATA) throws ConnectException, EOFException, SocketTimeoutException {
        ...
        try {
            ...
            if (urlConnection.getResponseMessage().equalsIgnoreCase("OK"))
                responseJSON = convertResponseToJSON(urlConnection.getInputStream());
            else
                responseJSON = convertResponseToJSON(urlConnection.getErrorStream());
